...
| Diagnostics | Implemented | Native diagnostics expose island URL/title/status/error and service probes. `native/brave-host/test-protocol.ps1` performs a bounded real-browser check of state, burst-poll responsiveness, correlated evaluation results, cookies, navigation, Back, and cleanup. Windows Event Viewer evidence is used to distinguish host hangs from renderer crashes. | Extend site-specific diagnostics only when runtime failures show missing evidence. |
