...
| Playback | Partial | Native uses Qt Multimedia for direct HLS feeds, treats YouTube feeds as browser-backed Brave island feeds with watch-page and compact embed actions, shows source badges, exposes failure details, and applies bounded resolution/playback-start guards plus capped exponential recovery. YouTube resolution is generation-safe: forced retries supersede in-flight requests and late responses after browser fallback or card destruction are ignored. A bounded 30-second isolated D3D11 Direct run on 2026-07-12 reached `playing` and buffered for LCN and Euronews, exposed browser fallback for all four unavailable YouTube probes, and exited cleanly. | Needs long-run stability, forced reconnect, unsupported-format, real Brave playback, and audio validation on a session with an output device. |
| Audio policy | Partial | Native has a single-audio owner path and muted default tiles, rejects unknown owners, releases ownership when a card fails or is destroyed, and mutes the native owner before handing playback to the browser island. | Verify live handoff across multiple tiles and browser-island audio behavior. |
| Live zoom | Partial | Native exposes zoom/open controls for every tile, opens YouTube feeds through normal watch URLs with a compact embed fallback in the Brave island, and opens direct HLS in a lightweight hls.js island with muted autoplay, bounded loading, network/media recovery, and visible playback state. | Runtime validation must decide whether the sidecar island fully replaces an in-panel zoom card for live playback diagnostics. |
...
| Live feeds | Partial | Qt Multimedia HLS handles direct feeds; YouTube feeds use browser-backed Brave island playback when native manifest extraction is unavailable, with primary watch-page and compact embed actions. The catalog matches the Electron Bloomberg and France 24 IDs; every tile has bounded startup/recovery, generation-safe resolution, zoom handoff, source, failure, and manual retry paths. LCN and Euronews played and buffered successfully in a bounded isolated Direct run on 2026-07-12. Needs long-run, output-device audio, and Brave YouTube playback validation. |
