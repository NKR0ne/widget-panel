...
| Explicit browser boundary | Implemented | Native services use native detail surfaces where available. The Brave-host island remains available only behind explicit service/browser commands; Direct mode no longer launches it implicitly. System-browser launches use a validated, logged `http`/`https` controller command. |
...
| Web escalation | Partial | Explicit service commands can open a Brave-host sidecar with address, refresh, external open, close, loading/error state, timeout feedback, and correlated CDP scripts. Direct playback and unavailable YouTube tiles no longer depend on it. | Continue replacing generic zoom paths with native detail surfaces; retain web sessions only where authentication/site behavior requires them. |
...
| TradingView heatmap drilldown | Partial | Native opens chart and heatmap paths, accepts cookies only from exact TradingView domains, automatically syncs after capture, and refreshes existing-session watchlists every 60 seconds without resetting unchanged models. Existing-session refresh loaded two lists on 2026-07-12. The real sign-in URL passed a 65-second reparent/poll/soak test without a Windows crash event; an unexpected island-host exit now reopens the preserved page through a bounded retry. | Runtime validation must still confirm a fresh TradingView sign-in/capture and real heatmap/chart drilldown. |
| PressReader automation | Implemented | Native opens the catalog island, keeps credentials in Vault, injects/reinjects login and start-reading scripts through correlated CDP evaluation, submits once per page signature, pauses around trusted user interaction, detects session/login/rejection state, applies a two-hour rejection guardrail, bounds unanswered script requests, and exposes pause/resume/manual modes. CDP work now runs on a serialized worker, state polls are coalesced, localhost calls are bounded, and site changes navigate the existing shell instead of blocking its Win32 message pump. | Live credential/session validation remains; dark-mode page recoloring is presentation rather than a functional parity blocker. |
...
| Diagnostics | Implemented | Native diagnostics expose island URL/title/status/error and service probes. `native/brave-host/test-protocol.ps1` performs a bounded real-browser check of state, burst-poll responsiveness, correlated evaluation results, cookies, navigation, Back, and cleanup. Real PressReader and TradingView sign-in URL soaks plus Windows Event Viewer evidence distinguish host hangs from renderer crashes. | Extend site-specific diagnostics only when runtime failures show missing evidence. |
