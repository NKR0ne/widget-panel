...
| Area | Status | Notes |
|---|---|---|
| Documented build flow | Implemented | `build.ps1` defaults to NMake, supports `-Tests`, reuses compatible CMake state, skips the phony Qt QML target when app inputs are unchanged, provides `-Reconfigure`, and delegates `-Run` to bounded `launch.ps1`. The no-op test path was verified at 3.6 seconds on 2026-07-11. |
...
| Windows startup ownership | Implemented | The Electron and standalone-helper login entries were removed on 2026-07-11; the stable NMake release `qt-panel.exe` is now the sole panel startup entry and spawns its own helper. |
...
| PressReader automation | Implemented | Native opens the catalog island, keeps credentials in Vault, injects login/start-reading scripts through CDP, reinjects after ezproxy/publication navigation, submits once per page signature, pauses around trusted user interaction, exposes timed pause/resume and a 30-minute manual-login mode, and updates the cooldown countdown. | Live credential/session validation remains; dark-mode page recoloring is presentation rather than a functional parity blocker. |
