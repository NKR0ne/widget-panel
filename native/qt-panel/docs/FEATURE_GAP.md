...
| PressReader automation | Implemented | Native opens the catalog island, keeps credentials in Vault, evaluates guarded login/start-reading scripts through WebEngine callbacks, submits once per page signature, pauses around trusted interaction, detects session/login/rejection state, applies a two-hour rejection guardrail, bounds unanswered requests, and exposes pause/resume/manual modes. Chromium forced dark mode plus guarded DOM recoloring cover the catalog and proxy pages. | Live credential/session validation remains; an isolated proxy-page run rendered dark and exited cleanly on 2026-07-12. |
...
| PressReader | Implemented | Native card opens the configurable Qt WebEngine catalog island, migrates login to Vault, exposes URL/login controls, reinjects guarded auto-login/start-reading automation across navigation, pauses around user interaction, supports pause/resume/manual mode, and has diagnostics/open hooks. Isolated dark-mode navigation and clean shutdown passed on 2026-07-12; live credential/session validation remains. |
