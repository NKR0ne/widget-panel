...
| Camera | Partial | XProtect remains isolated with bounded requests, generation-safe cancellation, frame watchdogs, discovery/picker, and explicit credential fallback. The separate `camera-direct` card now decodes the Hikvision RTSP substream natively through Qt Multimedia, renders it in-card, stores its password in Credential Manager, never falls back to another endpoint, and only auto-connects a configuration after a frame has verified it. Unverified configurations have a persistent two-attempt application budget keyed to the credential fingerprint; the rejected fingerprint remains blocked across `Forget` and requires a two-step manual rearm. | The RTSP endpoint is reachable, but runtime testing returned `401 Unauthorized` for the supplied direct-camera credentials on 2026-07-13. Needs exact credential validation after the camera lockout clears, plus end-to-end XProtect gateway validation. |
