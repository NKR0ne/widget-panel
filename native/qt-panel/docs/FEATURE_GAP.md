...
| Reader escalation | Implemented | News cards open the native `ReaderOverlay`, which supports direct/feed/publisher/Jina/archive attempts, progress and source state, image inspection, parse diagnostics, and embedded/external browser escalation. | Runtime validate fallback ordering and blocked publishers; Electron's launch ghost is intentionally not a native parity requirement. |
| Web escalation | Implemented | Explicit service commands open an in-process Qt WebEngine surface with native address/history/reload/external/close controls, persistent sessions, loading/error state, script callbacks, forced dark mode, and bounded renderer recovery. Direct playback and unavailable YouTube tiles do not depend on it. | Retain web sessions only where authentication or site behavior requires them; prefer native detail surfaces elsewhere. |
...
| News feed cards | Implemented | Reading remains the default workspace, while a persisted `Lire`/`Cartes` switch restores the alternate category-card view. Each category card has a hero image, one-story rotation, dots, prev/next controls, and persisted height; selecting a story returns to the in-place reader with its category retained. | Needs runtime feed validation for slow/broken feeds, high-volume categories, item de-dupe behavior, and stale cache recovery. |
...
| Layout behavior | Implemented | Reading view uses category, article-list, and in-place reader spans across the configured mode width. The alternate card view uses exactly the configured 3-6 column count and scrolls all active categories vertically. Isolated three-column D3D11 runs validated both views with 11 live categories on 2026-07-17. | Continue runtime coverage for very long titles, broken feeds, and larger column counts. |
...
| Monitor stage parity | Implemented | Dedicated native Station uses the Electron 2-primary plus 3-secondary telemetry hierarchy, selector-aware card visibility, live/stale status, full-width responsive packing, stable in-place graph updates, and native CPU/GPU power rings plus temperature bars. A 12-second D3D11 live-telemetry run captured the complete stage without QML errors on 2026-07-12. | Alert-priority tuning and longer reconnect testing remain. |
...
| Playback | Partial | Native uses Qt Multimedia for direct HLS feeds, shows source/failure state, and applies bounded resolution/playback guards plus capped recovery. YouTube resolution is generation-safe; restricted/unavailable responses are cached for the session and represented without automatic retries or browser fallback. A bounded 30-second isolated D3D11 run reached `playing` and buffered for LCN and Euronews. | Needs long-run stability, forced reconnect, unsupported-format, and audio validation on a session with an output device. |
| Audio policy | Partial | Native has one audio owner across tiles and the native detail player. Opening detail pauses its source tile, starts muted, and closing detail releases ownership before the tile resumes. | Verify live handoff across multiple tiles on a session with an output device. |
| Live detail | Implemented | Direct HLS opens in a full-stage native Qt Multimedia detail surface with retry, playback state, audio toggle, Escape/close behavior, and source-tile pause/resume. YouTube exposes only an explicit system-browser command and never launches from the tile body. | Runtime screenshot and audio-device validation remain. |
...
| Image extraction | Implemented | Native resolves relative/protocol-relative URLs, filters chrome/tracking/placeholder images, selects the strongest `srcset`/`data-srcset` candidate, supports common lazy-source attributes and `<picture>` sources, deduplicates up to five article images, shows thumbnails, and opens a full image viewer. Regression coverage verifies filtering, candidate selection, and URL handling. | Expand runtime fixtures only when a publisher exposes an unsupported image convention. |
...
| Reader native UI | Implemented | The overlay has source labels, paragraph/image counters, thumbnails, full image viewer, paywall/challenge copy, parse attempts, progress, archive, external open, and embedded browser actions. | Final visual-density and long-content validation remains; Electron-specific launch choreography is intentionally excluded. |

## Zoomed Area / Web Island Gap Analysis

| Feature | Native Status | Electron Feature / Gap | Next Validation |
|---|---|---|---|
| Core zoom surface | Implemented | `WebIsland.qml` embeds `WebEngineView` in the native window with reserved mode space and a disk-backed `QQuickWebEngineProfile`. It replaces the Brave sidecar and preserves the public controller contract used by cards and services. | Continue credentialed site validation; generic pages no longer depend on an external browser process or localhost CDP transport. |
| Toolbar/navigation | Implemented | Native QML supports state-aware Back/Forward, reload, address entry, external open, close, URL/title/readiness/error state, loading feedback, same-view popups, and bounded renderer recovery. | Continue visual-density tuning from daily use. |
...
| TradingView heatmap drilldown | Partial | Native opens chart and heatmap paths, captures only exact-domain TradingView cookies directly from the persistent WebEngine profile, automatically syncs after capture, and refreshes existing-session watchlists every 60 seconds without resetting unchanged models. Existing-session refresh loaded two lists on 2026-07-12. The real sign-in page rendered and exited cleanly in an isolated embedded-browser run on 2026-07-12. | Runtime validation must still confirm a fresh TradingView sign-in/capture and real heatmap/chart drilldown. |
| PressReader automation | Implemented | `PressReaderService` owns a bounded state machine independently of the optional card, keeps library credentials in Vault, permits one login submission per page signature, applies a two-hour rejection guardrail, records the 48-hour access window, and disables automatic submission in named diagnostic profiles. Versioned resource scripts probe/login/start-reading without an unbounded page interval. The library proxy's malformed `url=^U` return field is repaired to the canonical PressReader target before the single submit, and an authenticated `menu.htm` landing receives one bounded continuation through its own PressReader link. | A credential-safe isolated run reached the real dark proxy login page without QML/JavaScript errors on 2026-07-18. Valid login, rejection, catalog, publication, and 48-hour renewal still need controlled credentialed validation. |
| Microsoft auth/browser | Implemented | Native Graph PKCE opens inside Qt WebEngine, validates OAuth `state`, closes the island after a successful callback, supports explicit cancellation, and surfaces token, callback-port, timeout, provider, and code-exchange errors in the Microsoft cards. Invalid refresh credentials are cleared so the user can recover through interactive sign-in. | Existing-token startup loaded agenda, mail, and To Do successfully on 2026-07-11. A fresh-account interactive sign-in still needs credentialed runtime validation without disturbing the working profile. |
| Diagnostics | Implemented | Native diagnostics expose island URL/title/status/error and service probes. `--diag-island-url` plus `--diag-fitmode` performs a bounded real-page run and saves `diag-web-island.png`; launcher failures include the real exit code. Synthetic light-page, TradingView sign-in, and PressReader proxy runs verify dark rendering and clean subprocess teardown. | Extend site-specific diagnostics only when runtime failures show missing evidence. |
...
| News | Implemented | OPML/RSS, default three-pane reading workspace, persisted category-carousel workspace, archive hook, feed-aware matrix overlay, and 3D stage source exist. Carousel settings hot-reload. |
...
| Live feeds | Partial | Qt Multimedia handles direct feeds in both tiles and native detail. YouTube remains represented but becomes external-only after network restriction or native-resolution failure; no implicit Brave path remains. The catalog matches Electron, and every native tile has bounded startup/recovery, generation-safe resolution, source, failure, and manual retry paths. | Needs long-run and output-device audio validation. |
| FM radio | Implemented | Radio Browser discovery/search, persistent filters, resizable scrolling station list, card-local anchored menus, Stations/Favorites tabs with saved stream details, favorite playback and circular next/previous navigation, persistent volume, and native Qt Multimedia playback. Favorites survive directory changes and restarts. | Service/QML regression tests and isolated startup in both windowed/composition hosts passed on 2026-09-09. Native desktop menu interaction and live audio remain user validation. |
| Workstation telemetry | Implemented | Named-pipe client exists, is activated by visible workstation cards, and is covered by diagnostics state. Persistent QML models keep graph/detail delegates alive between one-second samples to avoid card flicker. Live service registration and ten consecutive samples passed in the isolated Station capture on 2026-07-12. |
| Camera | Implemented | The separate `camera-direct` card decodes the Hikvision substream through an isolated FFmpeg 8.1.2 worker with RTSP forced over TCP, corrupt-packet discard, bounded socket/frame watchdogs, and automatic transport reconnect. It renders fixed-size frames in-card and keeps the verified stream warm while hidden for Starvis analysis. The password remains in Credential Manager; no endpoint fallback occurs, and only confirmed authentication rejection consumes the persistent two-attempt budget. The pinned FFmpeg archive is checksum verified by `install-ffmpeg.ps1` and staged beside the panel executable during builds. | Release build, tests, live FFmpeg child process, and first-frame delivery passed on 2026-09-01. Long-run observation under peak network congestion remains useful. |
| PressReader | Implemented | A global header command and optional Base summary card open a dedicated native spotlight across columns 4-6. It uses an isolated persistent WebEngine profile, official library entry point, native navigation/session/account controls, proxy-only dark styling, Vault credentials, one-attempt automation, manual mode, and credential-safe diagnostics. The feature no longer depends on the optional card being active. | Credentialed runtime validation remains for valid/rejected login, publication launch, session persistence, and 48-hour renewal. |
