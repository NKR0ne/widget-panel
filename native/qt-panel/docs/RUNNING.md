...
## Behavior
- Left-edge acrylic sidebar, 5px horizontal screen inset and 10px vertical
  work-area inset, always-on-top, no taskbar button. Horizontal bounds ignore
  the helper's reserved AppBar strip so full-width modes retain equal visible
  margins. The native window slides over desktop content at 390ms and is hidden
  only after it clears the screen edge.
