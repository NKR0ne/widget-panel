...
TestCase {
    ...
    function test_sharedCategoryOrderAndSelectionPersistence() {
        ...
        const row = findChild(stage, "newsCategoryRow1")
        const start = row.mapToItem(stage, 80, row.height / 2)
        mousePress(stage, start.x, start.y)
        mouseMove(stage, start.x, start.y - 10, 30)
        mouseMove(stage, start.x, start.y - 25, 30)
        mouseMove(stage, start.x, start.y - 43, 30)
        mouseRelease(stage, start.x, start.y - 43)
        tryCompare(News, "categories", ["Science", "Quebec"])
        verify(!stage.draggingCategory)
        compare(stage.categoryDragIndex, -1)
        verify(!findChild(stage, "newsCategoryUp1"))
        compare(stage.selectedCategory, "Science")
